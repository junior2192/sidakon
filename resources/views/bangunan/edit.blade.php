        ...
        <form action="{{ route('bangunan.edit', $bangunan) }}" method="post">
            ...
            <div class="form-group row">
                ...
                <select name="paket_id" id="paket_id" class="form-control">
                    ...
                    <option {{ $bangunan->paket_id == $paket->id ? 'selected' : '' }} value="{{ $paket->id }}">{{
                        $paket->name.' || '.$paket->tahun_anggaran }}</option>
                    @endforeach
                </select>
                @error('paket_id')
            ...
            <div class="form-group row">
                ...
                <input type="text" class="form-control" id="name" value="{{ old('name') ?? $bangunan->name }}"
                    name="name" required>
                @error('name')
                <div class="text-danger mt-2">{{ $message }}</div>
                @enderror
            ...
            <div class="form-group row">
                ...
                <select class="form-control" name="province_id" id="province_id" required>
                    ...
                    <option {{ $bangunan->province_id == $item->id ? 'selected': '' }} value="{{ $item->id }}">{{
                        $item->name }}
                    </option>
                    @endforeach
                </select>
            ...
            <div class="form-group row">
                ...
                <select class="form-control" name="city_id" id="city_id" required>
                    ...
                    <option {{ $bangunan->city_id == $item->id ? 'selected' : '' }} value="{{ $item->id }}">{{
                        $item->name }}
                    </option>
                    @endforeach
                </select>
            ...
            <div class="form-group row">
                ...
                <select class="form-control" name="district_id" id="district_id" required>
                    ...
                    <option {{ $bangunan->district_id == $item->id ? 'selected' : '' }} value="{{ $item->id }}">{{
                        $item->name }}
                    </option>
                    @endforeach
                </select>
            ...
            <div class="form-group row">
                ...
                <select class="form-control" name="village_id" id="village_id" required>
                    ...
                    <option {{ $bangunan->village_id == $item->id ? 'selected' : '' }} value="{{ $item->id }}">{{
                        $item->name }}
                    </option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <div class="form-check">
                    <input class="form-check-input" type="radio" name="status" id="status1" value="0" {{ (old('status')
                        ?? $bangunan->status) == '0' ? 'checked' : '' }}>
                    <label for="status1" class="form-check-label">Belum Terbangun</label>
                </div>
                <div class="form-check">
                    <input class="form-check-input" type="radio" name="status" id="status2" value="1" {{ (old('status')
                        ?? $bangunan->status) == '1' ? 'checked' : '' }}>
                ...
                <div>
                    ...
                    <select name="tahun_konstruksi" id="tahun_konstruksi" class="form-control col-sm-4">
                        ...
                        @for ($i = 1980; $i <= date('Y'); $i++) <option {{ $bangunan->tahun_konstruksi == $i ?
